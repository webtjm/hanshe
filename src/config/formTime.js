...
export default class DateFormat {
    ...
    set year(y) {
        if (this.y !== y) {
            this.__date.setFullYear(y);
            this.setTime(this.__date);
        }
    }
    get year() {
        return this.y;
    ...
    __replaceTime(format) {
        ...
        format = format.replace(reg, (mstr) => {
            switch (mstr) {
                case 'yyyy':
                    return this.y; // [0000, 9999]
                case 'YYYY':
                    return this.perNumToChinese(this.y); // [零零零零-九九九九]   
                case 'yy':
                    return ('' + this.y).slice(-2); // [00, 99]
                case 'YY':
                    return this.perNumToChinese(+('' + this.y).slice(-2)); // [零零-九九]    
                case 'mm':
                    return this.m; // [1, 12]
                case 'MM':
                    return this.numToChinese(this.m); // [一-十二]    
                case '0m':
                    return this.m < 10 ? '0' + this.m : this.m; // [01, 12]
                case 'dd':
                    return this.d; // [1, 31]
                case 'DD':
                    return this.numToChinese(this.d); // [一-三十一]  
                case '0d':
                    return this.d < 10 ? '0' + this.d : this.d; // [01, 31]
                case 'hh':
                    return this.h; // [0, 24]
                case 'HH':
                    return this.numToChinese(this.h); // [零-二十四]  
                case '0h':
                    return this.h < 10 ? '0' + this.h : this.h; // [00, 24]
                case 'ff':
                    return this.f; // [0, 60]
                case 'FF':
                    return this.numToChinese(this.f); // [零-六十]
                case '0F':
                    return this.f < 10 && this.f > 0 ? ('零' + this.numToChinese(this.f)) : this.numToChinese(this.f); // [零零-六十]   
                case '0f':
                    return this.f < 10 ? '0' + this.f : this.f; // [00, 60]
                case 'ss':
                    return this.s; // [0, 60]
                case 'SS':
                    return this.numToChinese(this.s); // [零-六十]
                case '0S':
                    return this.s < 10 && this.s > 0 ? ('零' + this.numToChinese(this.s)) : this.numToChinese(this.s); // [零零-六十]    
                case '0s':
                    return this.s < 10 ? '0' + this.s : this.s; // [00, 60]
                case 'w':
                    return this.w; // [7]
                case 'W':
                    return this.w == 7 ? '日' : this.numToChinese(this.w); // [天|日]
                case 'WT':
                    return this.w == 7 ? '天' : this.numToChinese(this.w); // [天|日]
            }
        });
        return format;
    }
                ...
                for (let i = 1, val; i < namesArr.length; i++) {
                    ...
                    switch (namesArr[i]) {
                        case 'yyyy':
                            ...
                            break; // [0000, 9999]
                        case 'YYYY':
                            resultObj.y = this.perChineseToNum(val);
                            break; // [零零零零-九九九九]   
                        case 'yy':
                            resultObj.y = +val + 2000;
                            break; // [00, 99]
                        case 'YY':
                            resultObj.y = this.perChineseToNum(val) + 2000;
                            break; // [零零-九九]    
                        case 'mm':
                            resultObj.m = +val;
                            break; // [1, 12]
                        case 'MM':
                            resultObj.m = this.chineseToNum(val);
                            break; // [一-十二]    
                        case '0m':
                            resultObj.m = +val;
                            break; // [01, 12]
                        case 'dd':
                            ...
                            break; // [1, 31]
                        case 'DD':
                            resultObj.d = this.chineseToNum(val);
                            break; // [一-三十一]  
                        case '0d':
                            resultObj.d = +val;
                            break; // [01, 31]
                        case 'hh':
                            ...
                            break; // [0, 24]
                        case 'HH':
                            resultObj.h = this.chineseToNum(val);
                            break; // [零-二十四]  
                        case '0h':
                            resultObj.h = +val;
                            break; // [00, 24]
                        case 'ff':
                            ...
                            break; // [0, 60]
                        case 'FF':
                            resultObj.f = this.chineseToNum(val);
                            break; // [零-六十]
                        case '0F':
                            resultObj.f = this.chineseToNum(val);
                            break; // [零零-六十]   
                        ...
                        case 'ss':
                            ...
                            break; // [0, 60]
                        case 'SS':
                            resultObj.s = this.chineseToNum(val);
                            break; // [零-六十]
                        case '0S':
                            resultObj.s = this.chineseToNum(val);
                            break; // [零零-六十]    
    ...
    */
    numToChinese(num) {
        ...
        if (num <= 10) { //处理0~9
            ...
            let low4Str = numStr.slice(-4),
                heigh4Str = numStr.slice(0, -4),
                isZero = '';
            ...
            let low8Str = numStr.slice(-8),
                heigh4Str = numStr.slice(0, -8),
                isZero = '';
